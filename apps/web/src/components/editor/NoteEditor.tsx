...
export const NoteEditor: React.FC<NoteEditorProps> = ({
  ...
  const insertWikilink = (targetPath: string) => {
    ...
    const newText =
      text.slice(0, triggerIndex) + replacement + text.slice(cursorPos);
    onChange(newText);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (showAutocomplete && filteredPaths.length > 0) {
      ...
      if (e.key === "ArrowUp") {
        e.preventDefault();
        setSelectedIndex(
          (prev) => (prev - 1 + filteredPaths.length) % filteredPaths.length,
        );
        return;
      }
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-paper-canvas relative">
      ...
      {showAutocomplete && (
        <div
          className="absolute z-20 w-80 max-h-60 overflow-y-auto neo-box bg-white border-2 border-ink-primary shadow-neo-lg flex flex-col font-mono text-xs"
          style={{ bottom: "24px", left: "24px" }} // Fallback positioning since we don't have caret coordinates
          role="listbox"
        ...
        >
          ...
          {filteredPaths.length === 0 ? (
            <div
              className="p-3 text-ink-muted text-center text-[11px]"
              role="alert"
            >
              ...
              {filteredPaths.map((path, idx) => (
                <li
                  ...
                  className={`p-2 flex items-center gap-2 cursor-pointer transition-colors border-b border-cream-border/50 ${
                    idx === selectedIndex
                      ? "bg-accent-acid text-ink-primary font-bold"
                      : "hover:bg-cream-shell"
                  }`}
                >
                  <FileText
                    size={13}
                    className="text-accent-cobalt flex-shrink-0"
                  />
