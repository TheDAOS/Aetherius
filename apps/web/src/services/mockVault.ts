...
const DEFAULT_FILES: Record<
  string,
  { content: string; sha: string; lastModified: string }
> = {
  "README.md": {
...
class MockVaultService {
  ...
  async listFiles(prefix?: string): Promise<FileListResponse> {
    ...
    this.files.forEach((data, path) => {
      ...
      if (parts.length === 1) {
        ...
        const dirPath = normalizedPrefix
          ? `${normalizedPrefix}${dirName}`
          : dirName;
        if (!dirs.has(dirPath)) {
          dirs.add(dirPath);
          entries.push({
  ...
  async updateFile(path: string, req: UpdateFileRequest): Promise<VaultFile> {
    ...
    if (req.expectedSha && req.expectedSha !== existing.sha) {
      throw new Error(
        `Conflict: expected SHA ${req.expectedSha} but found ${existing.sha}`,
      );
    }
