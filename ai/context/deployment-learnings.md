...
## 6. Supabase OAuth Redirect to Localhost
* **Issue:** In a production environment (e.g., Vercel), completing the GitHub OAuth flow redirects the user back to `http://localhost:3000/#access_token=...` instead of the production application.
* **Fix/Workaround:** By default, new Supabase projects set their **Site URL** to `http://localhost:3000`. To fix this, navigate to the Supabase Dashboard -> **Authentication** -> **URL Configuration**, and update the **Site URL** to the production domain (e.g., `https://aetherius.sanju.fyi`). Any secondary domains (like `.vercel.app` preview URLs) should be added to the **Redirect URLs** allowlist below it.
